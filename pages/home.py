...
def update_graph(selected, table_data):
    ...
    if selected == "Guindaste - sem dados":
        return html.Div("Nenhum dado disponível para esse guindaste.", className="text-warning")
    ...
    if selected == "Guindaste 90ton":
        ...
        try:
            if not df_table.empty:
                ...
                df_table["Raio_conv"] = pd.to_numeric(df_table.get("Raio"), errors="coerce")
                df_table["Lanca_conv"] = pd.to_numeric(df_table.get("Lanca"), errors="coerce")
                df_table["Carga_conv"] = pd.to_numeric(df_table.get("Carga"), errors="coerce")
                ...
                if not df_valid.empty:
                    ...
                    if ok_mask.any():
                        ...
                        name="Aprovados",
                        customdata=np.stack([x[ok_mask], y[ok_mask], carga_ponto[ok_mask], carga_grafico[ok_mask]], axis=-1),
                        hovertemplate=(
                            "<b>Ponto:</b> %{text}<br>"
                    ...
                    if nok_mask.any():
                        fig.add_trace(go.Scatter(
                            ...
                            marker=dict(size=10, color="red", symbol="x",
                                        line=dict(width=1, color="black")),
                            name="Reprovados",
                            customdata=np.stack([x[nok_mask], y[nok_mask], carga_ponto[nok_mask], carga_grafico[nok_mask]], axis=-1),
                            hovertemplate=(
                                "<b>Ponto:</b> %{text}<br>"
                                "<b>Raio (x):</b> %{customdata[0]:.2f} m<br>"
                                "<b>Altura (y):</b> %{customdata[1]:.2f} m<br>"
...
tab1Columns =  [
                {"name": "Ponto", "id": "Ponto", "editable": True},
                {"name": "Lança", "id": "Lanca", "editable": True, "type": "numeric"},
                {"name": "Raio", "id": "Raio", "editable": True, "type": "numeric"},
                {"name": "Carga Demandada", "id": "Carga", "editable": True, "type": "numeric"},
            ]

initial_data = [{'Ponto': 'Aquecedor Fab.', 'Lanca': 32, 'Raio': 12.50, 'Carga': 8.}]
tabela_vendas = TabelaDadosComponent(app, id_base='dados-iniciais',columns=tab1Columns, initial_data=initial_data )




def layout():
    """Retorna o layout completo da página Home."""
    return html.Div([
        dbc.Container([
            ...
            dbc.Row([
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody([
                            html.H4("Visão Geral", className="card-title"),

                            html.P(
                                "Bem-vindo ao Dashboard de exemplo CraneChoice.",
                                className="card-text"
                            ),

                            html.P(
                                "Nessa sessão serão apresentadas todas as funcionalidades disponíveis.",
                                className="card-text"
                            ),

                            html.P(
                                "Lembrando que essa é uma aplicação experimental "
                                "de uso exclusivo dos funcionários REALL.",
                                className="card-text"
                            ),

                            html.Hr(), # Separador
                            
                            # Adiciona o layout da tabela, chamando o método .layout()
                            tabela_vendas.layout(),

                            dropdown_comp.layout(),
                            
                            html.Div(id="div-grafico-operacional"), 
                        ]),
                        className="shadow"
                    ),
                    width=12
    ...
    ]) # Este é o único fechamento do html.Div inicial